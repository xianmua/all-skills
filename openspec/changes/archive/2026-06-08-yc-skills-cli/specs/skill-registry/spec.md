...
- **WHEN** user executes `yc-skills add-origin https://gitlab.internal.company.com/skills`
- **THEN** the origin is added to `~/.yc-skills/config.toml` under `[origins]`
- **AND** the CLI displays a success message with the origin URL

#### Scenario: Add origin with custom name
- **WHEN** user executes `yc-skills add-origin https://gitlab.internal.company.com/skills --name internal`
