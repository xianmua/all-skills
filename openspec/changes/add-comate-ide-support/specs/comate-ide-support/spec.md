...
- **WHEN** user executes `all-skills install fmt --comate --trae`
...
- **WHEN** user executes `all-skills install fmt --comate --clion`
- **THEN** the CLI displays an error about conflicting options
- **AND** neither option is applied

#### Scenario: --comate conflicts with --dir
- **WHEN** user executes `all-skills install fmt --comate --dir ./custom`
- **THEN** the CLI displays an error about conflicting options
- **AND** neither option is applied
