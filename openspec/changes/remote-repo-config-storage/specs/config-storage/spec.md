...
The system SHALL store configuration at `~/.yc-skills/config.toml` using the `dirs` crate to determine the user home directory.

#### Scenario: Config directory creation
- **WHEN** configuration file is first accessed
- **THEN** the system SHALL create `~/.yc-skills/` directory if it does not exist
...
- **WHEN** user runs `yc-skills add-origin https://example.com/skills`
- **THEN** the updated configuration SHALL be written to `~/.yc-skills/config.toml`

#### Scenario: Load configuration on startup
- **WHEN** yc-skills starts
- **THEN** it SHALL load configuration from `~/.yc-skills/config.toml`
...
- **WHEN** yc-skills starts with no existing configuration
- **THEN** a default configuration SHALL be created at `~/.yc-skills/config.toml`
