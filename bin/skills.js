...
/**
 * skills CLI wrapper
 * Cross-platform wrapper for skills Rust binary
 */
...
function getBinaryInfo() {
  ...
  const localDevBinary = path.join(__dirname, '..', 'target', 'release', 'skills');
  if (fs.existsSync(localDevBinary)) {
    return localDevBinary;
  }

  // Check for .exe in development
  const localDevExe = path.join(__dirname, '..', 'target', 'release', 'skills.exe');
  if (fs.existsSync(localDevExe)) {
    return localDevExe;
  }

  // Production: use bundled binary
  const binaryName = platform === 'windows' ? 'skills.exe' : 'skills';
